  ...
  async onMessage(conn: Connection, msg: WSMessage) {
    ...
    if (m.type === "chat")
      this.setState({
        ...
        messages: [
          ...this.state.messages,
          { userId: conn.state.userId, text: m.text, timestamp: Date.now() },
        ],
      });
  }
}
...
const weatherTool = tool({
  ...
  execute: async ({ city }) => `Weather: ${city} Sunny 72°F`,
});
...
export class StreamingAgent extends Agent<Env> {
  async onMessage(conn: Connection, msg: WSMessage) {
    const m = JSON.parse(msg as string);
    const stream = await new OpenAI({ apiKey: this.env.OPENAI_API_KEY }).chat.completions.create({
      model: "gpt-4",
    ...
    for await (const chunk of stream) {
      ...
      if (c) conn.send(JSON.stringify({ type: "chunk", content: c }));
    }
...
export class TaskAgent extends Agent<Env> {
  onStart() {
    ...
    this.schedule("0 * * * *", "syncData", {});
  }
  async dailyCleanup() {
    this.sql`DELETE FROM logs WHERE created_at < ${Date.now() - 86400000}`;
  }
  async syncData() {
    const data = await (await fetch(this.env.API_URL)).json();
    this.sql`INSERT INTO cache (data,updated_at) VALUES (${JSON.stringify(data)},${Date.now()})`;
  }
}
```
...
export class EmailAgent extends Agent<Env> {
  async onEmail(email: AgentEmail) {
    ...
    this
      .sql`INSERT INTO emails (from_addr,subject,body,received_at) VALUES (${from},${subject},${text},${Date.now()})`;
