...
const producers = ["api", "webhook"].map(
  (name) =>
...
const consumers = ["email", "analytics"].map(
  (name) =>
...
const worker = new cloudflare.WorkerScript(
  "worker",
  { accountId, name: "my-worker", content: workerContent },
  { dependsOn: [build] },
);
```
...
const analytics = isProd
  ? new cloudflare.WorkersKvNamespace("analytics", { accountId, title: "analytics" })
  : undefined;
const worker = new cloudflare.WorkerScript("worker", {
  accountId,
  name: "worker",
