...
const devWorker = new cloudflare.WorkerScript(
  "dev-worker",
  {
...
const prodWorker = new cloudflare.WorkerScript(
  "prod-worker",
  {
...
```typescript
const prodDb = new cloudflare.D1Database(
  "prod-db",
  { accountId, name: "production-database" },
  { protect: true },
); // Cannot delete without removing protect
```
