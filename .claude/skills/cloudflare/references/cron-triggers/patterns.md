...
export default {
  async scheduled(controller, env, ctx) {
    ...
    console.log(
      `[START] Cron ${controller.cron} at ${new Date(controller.scheduledTime).toISOString()}`,
    );
    try {
      ...
      ctx.waitUntil(
        notifyFailure(env.ALERT_WEBHOOK, { cron: controller.cron, error: error.message }),
      );
      throw error;
    }
  },
  ...
  viewer {
    accounts(filter: { accountTag: $accountTag }) {
      workersInvocationsAdaptive(
        filter: { scriptName: $workerName, eventType: "scheduled" }
        limit: 100
      ) {
        dimensions {
          datetime
