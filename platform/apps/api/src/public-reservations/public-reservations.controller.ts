...
export class PublicReservationsController {
  ...
  getReservation(@Param("id") id: string, @Query("token") token?: string) {
  ...
  async getReservationFormSubmissions(@Param("id") id: string, @Query("token") token?: string) {
    // SECURITY: Require signed token for public reservation access
    if (!token) {
      throw new BadRequestException("access token required");
    }
   ...
   */
  @Get("forms/:id")
  async getPublicForm(@Param("id") id: string, @Query("campgroundId") campgroundId: string) {
