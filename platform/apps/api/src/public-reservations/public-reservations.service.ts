...
} from "./dto/create-public-reservation.dto";
...
import { resolveDiscounts, type DiscountCandidate } from "../pricing-v2/discount-engine";
...
const isLodgingType = (value: string) =>
  [
    "cabin",
    "glamping",
    "hotel_room",
...
type TaxRuleWithInclusive = TaxRule & { inclusive?: boolean };
...
export class PublicReservationsService {
  ...
  private async enqueueAbandonedCartPlaybooks(
    campgroundId: string,
    guestId?: string | null,
    ...
    for (const pb of playbooks) {
      ...
      const offsetMinutes = pb.offsetMinutes ? Number(pb.offsetMinutes) : null;
  ...
  private async resolveReferralProgram(
    ...
    const orConditions: Prisma.ReferralProgramWhereInput[] = [];
    if (referralProgramId) orConditions.push({ id: referralProgramId });
    if (referralCode) {
      orConditions.push({ code: referralCode });
      orConditions.push({ linkSlug: referralCode });
    }
  ...
  private async computeTaxes(
    ...
    const taxRules: TaxRuleWithInclusive[] = await this.prisma.taxRule.findMany({
      where: {
        campgroundId,
    ...
    const inclusiveRules = taxRules.filter((rule) => rule.inclusive);
    const exclusiveRules = taxRules.filter((rule) => !rule.inclusive);
    const inclusiveRateTotal = inclusiveRules.reduce(
      (sum, rule) => sum + Number(rule.rate ?? 0),
      0,
    );
    const netBase =
      inclusiveRateTotal > 0 ? Math.round(taxableCents / (1 + inclusiveRateTotal)) : taxableCents;
    const inclusiveTaxesCents = Math.max(0, taxableCents - netBase);
    let taxesCents = inclusiveTaxesCents;
  ...
  async getAvailability(
    ...
    if (previewToken && campground) {
      const invite = await this.prisma.onboardingInvite.findUnique({
        ...
        include: {
          OnboardingSession: { include: { Campground: true } },
          Campground: true,
        },
      });
      const inviteCampground = invite?.OnboardingSession?.Campground || invite?.Campground;
    ...
    if (!isPreviewMode && (!campground.isBookable || campground.isExternal)) {
      throw new BadRequestException(
        campground.nonBookableReason || "This campground is view-only.",
      );
    }
    ...
    const sitesWithStatus = allSites.map((site) => {
      ...
      if (campgroundBlackedOut) {
        ...
        // Default cutoffs: RV/tent = 0 (no cutoff), lodging = 60 minutes
        if (cutoffMinutes === null || cutoffMinutes === undefined) {
          if (isLodgingType(siteType)) {
  ...
  async getQuote(slug: string, dto: PublicQuoteDto) {
    ...
    if (dto.membershipId) {
      ...
      if (
        membership &&
        membership.MembershipType?.discountPercent &&
        membership.MembershipType.isActive
      ) {
        candidates.push({
          id: membership.id,
  ...
  async createReservation(dto: CreatePublicReservationDto) {
    ...
    const requestedRigLength =
      dto.equipment?.length !== undefined && dto.equipment?.length !== null
    ...
    if (dto.needsAccessible && !(site.accessible || site.SiteClass?.accessible)) {
      throw new BadRequestException(
        "Accessible site required; please choose an ADA-accessible site.",
      );
    }

    // Get price quote (handles promo/membership + cap)
    let quote: Awaited<ReturnType<PublicReservationsService["getQuote"]>> = await this.getQuote(
      dto.campgroundSlug,
      {
        siteId: siteId,
    ...
    const totalAfterDiscount =
      quote.totalAfterDiscountCents ?? Math.max(0, subtotal - discountCents);
    ...
    const now = new Date();
    const leadTimeDays = Math.floor((arrival.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
      ...
      try {
        ...
        const reservation = await this.prisma.reservation.create({
          data: {
            ...
            petTypes: toNullableJsonInput(dto.petTypes ?? null),
            ...
            additionalGuests: dto.additionalGuests
              ? JSON.parse(JSON.stringify(dto.additionalGuests))
              : null,
            childrenDetails: dto.childrenDetails
              ? JSON.parse(JSON.stringify(dto.childrenDetails))
              : null,
        ...
        const reservationWithRelations = await this.prisma.reservation.findUnique({
        ...
        try {
          await this.policies.applyPoliciesToReservation({
            ...
            guest: reservationWithRelations.Guest,
            site: reservationWithRelations.Site,
            siteClass: reservationWithRelations.Site?.SiteClass,
            channel: "online",
        ...
        const cg = reservationWithRelations.Campground;
        ...
        // Format dates nicely
        const arrivalFormatted = reservation.arrivalDate.toLocaleDateString("en-US", {
          weekday: "long",
          month: "long",
          day: "numeric",
          year: "numeric",
        });
        const departureFormatted = reservation.departureDate.toLocaleDateString("en-US", {
          weekday: "long",
          month: "long",
          day: "numeric",
          year: "numeric",
        ...
        const formsToComplete = afterBookingForms.filter((f) => {
          const showAtArray = Array.isArray(f.showAt) ? f.showAt : [];
        ...
        try {
          await this.emailService.sendEmail({
            to: reservationWithRelations.Guest.email,
            subject: `Reservation Confirmed: ${reservationWithRelations.Campground.name}`,
            html: `
                            <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
                                <div style="text-align: center; margin-bottom: 30px;">
                                    <div style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); border-radius: 12px; padding: 32px; margin-bottom: 24px;">
                                        ...
                                        <p style="color: rgba(255,255,255,0.9); margin: 0; font-size: 16px;">${reservationWithRelations.Campground.name}</p>
                                    </div>
                                </div>
                                ...
                                <div style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px; padding: 24px; margin-bottom: 20px;">
                                    <p style="margin: 0 0 20px 0; color: #475569; font-size: 16px;">
                                        Hi ${reservationWithRelations.Guest.primaryFirstName},<br/>
                                    ...
                                    <div style="background: #f8fafc; border-radius: 8px; padding: 20px; margin-bottom: 20px;">
                                        <table style="width: 100%; border-collapse: collapse;">
                                            <tr>
                                                ...
                                                <td style="padding: 8px 0; color: #0f172a; text-align: right; border-bottom: 1px solid #e2e8f0; font-weight: 600;">${reservationWithRelations.Site.siteNumber}</td>
                                            </tr>
                                            <tr>
                                ...
                                <div style="text-align: center; padding: 20px 0; border-top: 1px solid #e2e8f0;">
                                    <p style="color: #64748b; font-size: 13px; margin: 0 0 8px 0;">
                                        Questions about your stay? Contact ${reservationWithRelations.Campground.name}
                                    </p>
        ...
        throw new BadRequestException(`Failed to create reservation: ${message}`);
      }
    });
  }
  ...
  async kioskCheckIn(id: string, upsellTotalCents: number, kioskToken: string) {
    ...
    if (reservation.waiverRequired) {
      ...
      if (!signed) {
        ...
        const signingUrl =
          signatureResult && typeof signatureResult === "object" && "signingUrl" in signatureResult
            ? typeof signatureResult.signingUrl === "string"
    ...
    if (balanceDue > 0) {
      ...
      try {
        ...
        const message = error instanceof Error ? error.message : "Unknown error";
        this.logger.error(`Kiosk - Payment failed for ${id}:`, message);
        throw new BadRequestException(
    ...
    try {
      ...
      if (balanceDue > 0) {
        try {
          ...
          this.logger.error(
            "Kiosk - Failed to send payment receipt email:",
            emailError instanceof Error ? emailError.stack : emailError,
          );
        }
      }
  ...
  async getReservation(id: string, token?: string) {
    ...
    let discountsInfo: {
      appliedDiscounts: { id: string; type: string; amountCents: number; capped: boolean }[];
      rejectedDiscounts: { id: string; reason: string }[];
      discountCapped: boolean;
    } = { appliedDiscounts: [], rejectedDiscounts: [], discountCapped: false };
    try {
    ...
    return {
      ...reservation,
      publicAccessToken: this.buildPublicReservationToken(id),
      ...discountsInfo,
    };
  }
}
