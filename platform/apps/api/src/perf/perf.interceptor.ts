...
export class PerfInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler<unknown>): Observable<unknown> {
    ...
    const campgroundId = Array.isArray(campgroundHeader) ? campgroundHeader[0] : campgroundHeader ?? null;
    // Extract and validate client IP to prevent spoofing via x-forwarded-for
    const ip = extractClientIpFromRequest(req);
    const accessLogsEnabled = process.env.ACCESS_LOGS_ENABLED !== "false" && process.env.NODE_ENV !== "test";
