...
export class BatchInventoryService {
  ...
  async createBatch(campgroundId: string, dto: CreateBatchDto, actorUserId: string) {
  ...
  async adjustBatch(id: string, campgroundId: string, dto: AdjustBatchDto, actorUserId: string) {
    const batch = await this.prisma.inventoryBatch.findUnique({ where: { id } });
    if (!batch) throw new NotFoundException("Batch not found");
    if (batch.campgroundId !== campgroundId) {
      throw new ForbiddenException("Access denied to this batch");
  ...
  async disposeBatch(id: string, campgroundId: string, dto: DisposeBatchDto, actorUserId: string) {
    const batch = await this.prisma.inventoryBatch.findUnique({
      where: { id },
      include: {
  ...
  async allocateFEFO(
    ...
    for (const batch of batches) {
      ...
      if (batch.expirationDate && batch.expirationDate < now) {
        if (!options?.allowExpired && !options?.previewOnly) {
          throw new ExpiredBatchException(productId, batch.id, batch.expirationDate);
        }
    ...
    if (remaining > 0 && !options?.previewOnly) {
      ...
      throw new InsufficientBatchInventoryException(productId, requestedQty, totalAvailable);
    }
