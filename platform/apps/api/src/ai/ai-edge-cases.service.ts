...
export class AiEdgeCasesService {
  ...
  async analyzeQuery(
    ...
    if (!normalizedQuery || normalizedQuery.length < 3) {
      return {
        ...
        type: "vague",
        clarificationNeeded:
          'Your query is too short. What are you looking for? For example, you could ask "Show me RV sites available next weekend" or "Find a pet-friendly cabin for July 4th".',
        originalQuery: query,
        confidence: 1.0,
      };
    }
    ...
    const siteTypeCheck = await this.checkSiteTypes(
      campgroundId,
      correctedQuery,
      context?.siteTypes,
    );
   ...
   * Check for impossible date requests
   */
  private async checkDates(query: string, minDate?: Date, maxDate?: Date): Promise<EdgeCaseResult> {
    ...
    if (maxDate) {
      const farFuturePatterns = [/20\d{2}/];
  ...
  private checkVagueQuery(query: string): EdgeCaseResult {
    ...
    for (const { pattern, type } of vaguePatterns) {
      if (pattern.test(query)) {
        ...
        switch (type) {
          case "greeting":
            clarification =
              "Hello! I can help you find and book a campsite. What dates are you looking for, and what type of site do you need (RV, tent, cabin)?";
            suggestions = [
          ...
          case "no_criteria":
            clarification =
              "I need a bit more information to help you. When would you like to stay, and what type of site are you looking for?";
            suggestions = [
          ...
          case "no_dates":
            clarification =
              "To check availability, I need to know your dates. When are you planning to visit?";
            suggestions = ["Next weekend", "June 15-20", "2 weeks from now for 3 nights"];
            break;
          default:
    ...
    const hasDateIndicator =
      /\b(today|tomorrow|tonight|next|this|on|for|from|until|through|january|february|march|april|may|june|july|august|september|october|november|december|\d{1,2}\/\d{1,2}|\d{4}-\d{2}-\d{2}|weekend|week|month)\b/i.test(
        query,
      );
  ...
  private checkOutOfScope(query: string): EdgeCaseResult {
    const outOfScopePatterns = [
      {
        pattern: /weather forecast|temperature|rain/i,
        response: "weather information",
        suggestion: "check a weather service like weather.com",
      },
      {
        pattern: /restaurant|food|dining|eat/i,
        response: "restaurant recommendations",
        suggestion: "ask about our camp store or on-site amenities instead",
      },
      {
        pattern: /directions|how (do i|to) get|drive|map/i,
        response: "driving directions",
        suggestion: "use Google Maps or your GPS with our address",
      },
      ...
      {
        pattern: /wifi password|internet code/i,
        response: "WiFi credentials",
        suggestion: "ask at the front desk when you check in",
      },
      { pattern: /cancel my|refund|money back/i, response: null, suggestion: null }, // Allow - this is a reservation action
      {
        pattern: /complaint|manager|supervisor/i,
        response: "customer service escalation",
        suggestion: "contact us directly at our main phone number or email",
      },
      {
        pattern: /other campground|competitor|compare/i,
        response: "information about other campgrounds",
        suggestion: "search campground directories or review sites",
      },
    ];
  ...
  private checkAmbiguity(query: string): EdgeCaseResult {
    ...
    const hasSpecificDate =
      /\b(next|this|coming|following|\d{1,2}[\/\-]\d{1,2}|january|february|march|april|may|june|july|august|september|october|november|december \d)/i.test(
        query,
      );
    ...
    for (const [amenity, aliases] of Object.entries(AMENITY_ALIASES)) {
      for (const alias of aliases) {
        if (
          ...
          this.levenshteinDistance(normalizedInput, alias) <= 2
        ) {
