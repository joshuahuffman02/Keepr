...
export class AiReplyAssistService {
  ...
  async generateReplySuggestions(
    ...
    const anonymizationLevel = this.normalizeAnonymizationLevel(campground.aiAnonymizationLevel);
...
<reply text>`;
  ...
  private buildUserPrompt(
    ...
    if (context.siteName || context.dates) {
      ...
      if (context.dates)
        prompt += `- Dates: ${context.dates.arrival} to ${context.dates.departure}\n`;
    }
  ...
  private parseSuggestions(content: string): ReplySuggestion[] {
    ...
    const regex =
      /SUGGESTION \d+ \[CONFIDENCE: (high|medium|low)\] \[TONE: (professional|friendly|formal)\]\s*\n([\s\S]*?)(?=SUGGESTION \d+|$)/gi;
