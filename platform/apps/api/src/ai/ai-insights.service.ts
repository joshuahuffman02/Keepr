...
Action: <suggested action or "N/A">`;
  ...
  private async gatherCampgroundData(campgroundId: string): Promise<Record<string, unknown>> {
    ...
    const [campground, recentReservations, siteStats, revenueStats] = await Promise.all([
    ...
    const occupancyRate =
      totalSites > 0 ? Math.round((upcomingReservations / totalSites) * 100) : 0;
  ...
  private parseAutoInsights(content: string): AutoInsight[] {
    ...
    for (const block of insightBlocks) {
      ...
      const categoryMatch = block.match(
        /Category:\s*(revenue|occupancy|guest_satisfaction|operations|anomaly)/i,
      );
      ...
      if (titleMatch && summaryMatch) {
        ...
        insights.push({
          ...
          priority:
            priorityValue && isAutoInsightPriority(priorityValue) ? priorityValue : "medium",
          category:
            categoryValue && isAutoInsightCategory(categoryValue) ? categoryValue : "operations",
          actionable: actionableMatch?.[1]?.toLowerCase() === "true",
          suggestedAction:
            actionMatch?.[1]?.trim() !== "N/A" ? actionMatch?.[1]?.trim() : undefined,
        });
      }
    }
