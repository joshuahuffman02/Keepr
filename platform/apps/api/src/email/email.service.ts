...
export class EmailService {
  ...
  constructor(private readonly prisma: PrismaService) {
    ...
    if (this.resendApiKey) {
      ...
      this.logger.warn(
        "Resend, SMTP, and Postmark not configured; falling back to console logging emails.",
      );
    }
  }
  ...
  ): Promise<{ providerMessageId?: string; provider?: string; fallback?: string }> {
    ...
    const tryPostmark = async () => {
      ...
      if (!res.ok) {
        throw new BadRequestException(
          `Postmark send failed: ${res.status} ${JSON.stringify(data)}`,
        );
      }
...
================================================================================
  ...
  async sendPaymentReceipt(options: PaymentReceiptOptions): Promise<void> {
    ...
    const hasLines =
      lineItems.length > 0 || options.taxCents !== undefined || options.feeCents !== undefined;
    const totalCents =
      options.totalCents ??
      (lineItems.length
    ...
    if (!options.guestEmail || !options.guestEmail.includes("@")) {
      this.logger.warn(
        `Cannot send payment receipt: invalid or missing email address for guest ${options.guestName}`,
      );
      return;
    }
