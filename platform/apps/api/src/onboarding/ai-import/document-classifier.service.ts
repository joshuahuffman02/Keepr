...
const ENTITY_COLUMN_PATTERNS: Record<TargetEntity, string[]> = {
    sites: [
        'site', 'site_number', 'site_num', 'site_id', 'spot', 'space',
        'site_type', 'type', 'category', 'class',
        'hookups', 'water', 'sewer', 'electric', 'amps',
        'max_length', 'rig_length', 'pull_through',
        'occupancy', 'max_occupancy', 'capacity',
    ],
    guests: [
        'first_name', 'last_name', 'name', 'guest_name',
        'email', 'phone', 'mobile', 'cell',
        'address', 'city', 'state', 'zip', 'postal',
        'rig_type', 'vehicle', 'plate',
    ],
    reservations: [
        'arrival', 'check_in', 'check-in', 'start_date',
        'departure', 'check_out', 'check-out', 'end_date',
        'confirmation', 'booking', 'reservation_id',
        'total', 'amount', 'paid', 'balance',
        'nights', 'duration', 'status',
    ],
    rates: [
        'rate', 'price', 'nightly', 'daily',
        'weekly', 'monthly', 'seasonal',
        'effective', 'start_date', 'end_date',
        'minimum', 'maximum', 'discount',
    ],
    policies: [
        'policy', 'rule', 'terms', 'conditions',
        'cancellation', 'refund', 'deposit',
        'check_in', 'check_out', 'quiet_hours',
    ],
};
...
export class DocumentClassifierService {
     ...
     */
    classifyByColumns(columns: string[]): { suggestedEntity: TargetEntity | null; confidence: number; contentType: ContentType } {
        const normalizedColumns = columns.map(c =>
            c.toLowerCase().replace(/[^a-z0-9]/g, '_').replace(/_+/g, '_')
        );
    ...
    async classifyWithAI(
        ...
        fileName: string,
    ): Promise<{ contentType: ContentType; suggestedEntity: TargetEntity | null; confidence: number; reasoning: string }> {
        if (!this.openai) {
            return {
                contentType: 'unknown',
        ...
        try {
            ...
            if (typeof content === 'string') {
                ...
                messages = [
                    ...
                    { role: 'user', content: `Classify this document content from "${fileName}":\n\n${content.slice(0, 3000)}` },
                ];
            } else {
                // Image content
                messages = [
