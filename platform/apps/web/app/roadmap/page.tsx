            ...
            {phase.milestones.map((milestone) => (
              <div key={milestone.id} className="flex items-start gap-3 py-1.5">
                ...
                <div className="flex-1 min-w-0">
                  <div
                    className={`text-sm font-medium ${milestone.status === "completed" ? "text-slate-600" : "text-slate-900"}`}
                  >
                    {milestone.name}
                  </div>
...
export default function RoadmapPage() {
  ...
  const platformRole =
    isRecord(whoami?.user) && typeof whoami.user.platformRole === "string"
      ? whoami.user.platformRole
      : undefined;

  const isStaff =
    !!platformRole || (whoami?.user?.memberships && whoami.user.memberships.length > 0);
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-slate-600 max-w-2xl mx-auto">
            Our living roadmap showing what we&apos;ve built, what we&apos;re building, and
            what&apos;s coming next.
          </p>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-600">
                Sequenced phases from the Dec 09, 2025 roadmap refresh.
              </p>
            </div>
