...
const STATUS_CONFIG: Record<
  string,
  { bg: string; icon: typeof CheckCircle; border: string; shadow: string }
> = {
  confirmed: {
...
export function ReservationPill({
  ...
  const lockedAccent = reservation.siteLocked ? "border-status-warning/30" : "";
  ...
  return (
    <div
      className={cn(
        ...
        lockedAccent,
        isHighlighted
          ? "ring-2 ring-white/60 ring-offset-2 ring-offset-background z-20 shadow-lg"
          : "z-10",
        isDragging && !isBeingDragged && "pointer-events-none opacity-40 grayscale-[0.5]",
        isBeingDragged &&
          "ring-2 ring-status-info ring-offset-1 opacity-90 scale-[1.02] shadow-2xl z-50",
      )}
      style={style}
    ...
    >
      {reservation.siteLocked && (
        <span
          className="absolute left-0 top-0 bottom-0 w-2 bg-status-warning rounded-l-lg"
          aria-hidden="true"
        />
      )}
      <div className="flex items-center min-w-0 w-full">
        ...
        <div className="flex flex-col leading-none min-w-0 flex-1">
          ...
          {style.width && parseInt(style.width.toString()) > 120 && (
            <span className="text-[9px] opacity-70 truncate font-mono mt-0.5">
              ${total.toFixed(0)}
            </span>
          )}
        </div>
        {showSignals && (
          <div className="ml-2 flex gap-1 items-center flex-shrink-0">
            ...
            {hasConflict && (
              <AlertTriangle className="h-2.5 w-2.5 text-rose-100/80" aria-label="Conflict" />
            )}
          </div>
        )}
      </div>
