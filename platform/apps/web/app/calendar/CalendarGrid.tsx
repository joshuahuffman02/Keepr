...
export function CalendarGrid({ data, onSelectionComplete, onReservationMove }: CalendarGridProps) {
  ...
  const handleDragEnd = useCallback(
    async (siteId: string | null, dayIdx: number | null) => {
      const drag = dragRef.current;
  ...
  const handleGridPointerMove = useCallback(
    ...
    [
      ...
      resolveDragTarget,
      handleDragEnter,
      reservationDrag.isDragging,
      updateReservationDrag,
    ],
  );

  // Handler for starting a reservation drag (move or extend)
  const handleReservationDragStart = useCallback(
    (reservationId: string, mode: ReservationDragMode) => {
      const reservation = (reservations.data || []).find(
        (r: CalendarReservation) => r.id === reservationId,
      );
  ...
  const handleReservationDragEnd = useCallback(() => {
    ...
    if (
      newArrivalStr !== originalArrival ||
      newDepartureStr !== originalDeparture ||
      newSiteId !== originalSiteId
    ) {
  ...
  return (
    ...
    >
      <div className="overflow-x-auto">
        ...
        >
          <div className="px-4 py-3 sticky left-0 z-30 bg-muted border-r border-border">Sites</div>
          {days.map((d, idx) => (
            <div
              key={idx}
            ...
            >
              {d.label}
              {d.isToday && (
                <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-status-info rounded-full mx-2" />
              )}
            </div>
          ))}
        </div>
