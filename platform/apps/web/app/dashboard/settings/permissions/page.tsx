...
export default function PermissionsPage() {
  ...
  const [ruleForm, setRuleForm] = useState({ role: "manager", resource: "audit", action: "export", fields: "", effect: "allow" });
  const [approvalForm, setApprovalForm] = useState({ action: "export_pii", resource: "audit", targetId: "", justification: "" });
  ...
  const upsertRule = useMutation({
    mutationFn: () =>
      apiClient.upsertPermissionRule({
        ...
        fields: ruleForm.fields ? ruleForm.fields.split(",").map((f) => f.trim()).filter(Boolean) : []
      }),
    ...
    <div className="space-y-6">
        ...
        <div className="card p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
            <div>
              ...
              <Select value={ruleForm.role} onValueChange={(v) => setRuleForm((f) => ({ ...f, role: v }))}>
            ...
            <div>
              ...
              <Input value={ruleForm.resource} onChange={(e) => setRuleForm((f) => ({ ...f, resource: e.target.value }))} />
            </div>
            <div>
              <Label className="text-sm">Action</Label>
              <Input value={ruleForm.action} onChange={(e) => setRuleForm((f) => ({ ...f, action: e.target.value }))} />
            </div>
            <div>
              <Label className="text-sm">Fields (comma separated)</Label>
              <Input value={ruleForm.fields} onChange={(e) => setRuleForm((f) => ({ ...f, fields: e.target.value }))} />
            </div>
            <div>
              <Label className="text-sm">Effect</Label>
              <Select value={ruleForm.effect} onValueChange={(v) => setRuleForm((f) => ({ ...f, effect: v }))}>
                <SelectTrigger><SelectValue placeholder="Effect" /></SelectTrigger>
                <SelectContent>
        ...
        <div className="card p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            <div>
              <Label className="text-sm">Action</Label>
              <Input value={approvalForm.action} onChange={(e) => setApprovalForm((f) => ({ ...f, action: e.target.value }))} />
            </div>
            <div>
              <Label className="text-sm">Resource</Label>
              <Input value={approvalForm.resource} onChange={(e) => setApprovalForm((f) => ({ ...f, resource: e.target.value }))} />
            </div>
            <div>
              <Label className="text-sm">Target ID</Label>
              <Input value={approvalForm.targetId} onChange={(e) => setApprovalForm((f) => ({ ...f, targetId: e.target.value }))} />
            </div>
            <div>
              <Label className="text-sm">Justification</Label>
              <Input value={approvalForm.justification} onChange={(e) => setApprovalForm((f) => ({ ...f, justification: e.target.value }))} />
            </div>
          </div>
