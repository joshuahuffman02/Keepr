          ...
          <div className="bg-status-warning/10 border border-status-warning/20 rounded-lg p-4">
            ...
            <div className="grid gap-2">
              {lowStockProducts.map((product) => (
                <div
                  key={product.id}
                  className="flex items-center justify-between bg-card p-2 rounded border border-status-warning/20"
                >
                  <span className="font-medium text-foreground">{product.name}</span>
                  <div className="flex items-center gap-4">
                    <span className="text-sm text-muted-foreground">
                      Current:{" "}
                      <span className="font-bold text-status-error">{product.stockQty}</span>
                    </span>
                    <span className="text-sm text-muted-foreground">
