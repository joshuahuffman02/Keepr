      ...
      <DashboardShell>
        ...
        <Card className="border-border">
          <CardContent className="py-16 text-center">
            ...
            <h3 className="text-lg font-medium text-foreground mb-2">Select a campground</h3>
        ...
        >
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <h3 className="font-semibold text-status-error">
                    Action needed: {metrics.urgentDisputes.length} dispute
                    {metrics.urgentDisputes.length > 1 ? "s" : ""} due soon
                  </h3>
          ...
          <motion.div variants={shouldReduceMotion ? {} : item}>
            <div className="grid gap-4 sm:grid-cols-3">
              ...
              <motion.div
                whileHover={
                  shouldReduceMotion ? {} : { y: -2, boxShadow: "0 8px 16px rgba(0,0,0,0.1)" }
                }
              ...
              >
                ...
                {metrics.nextPayout ? (
                  <>
                    ...
                    <div className="flex items-center gap-1 text-sm text-muted-foreground mt-1">
                      ...
                      {metrics.nextPayout.arrivalDate
                        ? formatDistanceToNow(new Date(metrics.nextPayout.arrivalDate), {
                            addSuffix: true,
                          })
                        : "Processing"}
                    </div>
                  ...
                  <>
                    <div className="text-lg font-medium text-foreground">No pending payouts</div>
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  {metrics.revenueChange !== 0 && (
                    ...
                    >
                      {metrics.revenueChange > 0 ? "+" : ""}
                      {metrics.revenueChange.toFixed(0)}% vs last month
                    </Badge>
                  )}
                </div>
                <div className="text-xs font-medium uppercase text-muted-foreground mb-1">
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div
                    className={`rounded-full p-2 ${
                      metrics.disputeSummary.open > 0 ? "bg-status-warning/10" : "bg-muted"
                    }`}
                  >
                    <Shield
                      className={`h-5 w-5 ${
                ...
                {metrics.disputeSummary.total > 0 ? (
                      ...
                      <div className="mt-3">
                        <div className="h-2 bg-muted rounded-full overflow-hidden">
                          <motion.div
                            initial={{ width: 0 }}
                            animate={{
                              width: `${(metrics.disputeSummary.won / metrics.disputeSummary.total) * 100}%`,
                            }}
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          {Math.round(
                            (metrics.disputeSummary.won / metrics.disputeSummary.total) * 100,
                          )}
                          % win rate
                        </p>
                      </div>
                  ...
                  <div className="text-lg font-medium text-foreground">No disputes</div>
                )}
              </motion.div>
            </div>
          </motion.div>
          ...
          {/* Navigation Cards */}
          <motion.div variants={shouldReduceMotion ? {} : item}>
            <h2 className="text-lg font-semibold text-foreground mb-4">Manage Finances</h2>
            <div className="grid gap-4 sm:grid-cols-3">
              {/* Payouts Link */}
              <Link href="/finance/payouts">
                <motion.div
                  whileHover={
                    shouldReduceMotion
                      ? {}
                      : { scale: 1.02, boxShadow: "0 8px 16px rgba(0,0,0,0.1)" }
                  }
                ...
                >
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <h3 className="font-semibold text-foreground">Payouts & Deposits</h3>
                  </div>
              ...
              <Link href="/finance/disputes">
                ...
                >
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <h3 className="font-semibold text-foreground">Payment Disputes</h3>
                    {metrics.disputeSummary.open > 0 && (
                      <Badge className="bg-status-warning/15 text-status-warning border-status-warning">
                        {metrics.disputeSummary.open}
                      </Badge>
                  ...
                  {metrics.disputeSummary.open > 0 && (
                    <p className="text-xs text-status-warning">
                      {metrics.disputeSummary.open} dispute
                      {metrics.disputeSummary.open > 1 ? "s" : ""} need response
                    </p>
                  )}
                  <div
              ...
              <Link href="/finance/gift-cards">
                ...
                >
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <h3 className="font-semibold text-foreground">Gift Cards & Credits</h3>
                  </div>
                  <p className="text-sm text-muted-foreground mb-3">
            ...
            <motion.div variants={shouldReduceMotion ? {} : item}>
              <Card className="border-border">
                <CardHeader>
                  ...
                  <CardDescription className="">Your latest financial transactions</CardDescription>
                </CardHeader>
                <CardContent>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              {payout.arrivalDate
                                ? formatDistanceToNow(new Date(payout.arrivalDate), {
                                    addSuffix: true,
                                  })
                                : formatDistanceToNow(new Date(payout.createdAt || Date.now()), {
                                    addSuffix: true,
                                  })}
                            </p>
                          </div>
                        ...
                        <span
                          className={`font-semibold ${
                            payout.status === "paid" ? "text-status-success" : "text-foreground"
                          }`}
                        >
