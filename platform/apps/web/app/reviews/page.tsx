...
export default function ReviewsPage() {
  ...
  const { data, isLoading, error } = useQuery<Review[]>({
    ...
    queryFn: () =>
      apiClient.getAdminReviews(campgroundId!, statusFilter === "all" ? undefined : statusFilter),
  ...
  const filteredReviews = reviews.filter((review) => {
    if (searchQuery) {
      ...
      const matchesGuest = `${review.guest?.primaryFirstName} ${review.guest?.primaryLastName}`
        .toLowerCase()
        .includes(query);
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            {filteredReviews.map((review) => (
              <Card
                key={review.id}
                className={review.status === "pending" ? "border-amber-200" : ""}
              >
                <CardContent className="pt-6">
                      ...
                      <div className="text-sm text-muted-foreground">
                        ...
                        {review.reservation?.id && (
                          <span className="text-muted-foreground">
                            {" "}
                            &middot; Reservation {review.reservation.id.slice(0, 8)}
                          </span>
                        )}
                      </div>
                      ...
                      {review.replies && review.replies.length > 0 && (
                        <div className="bg-muted rounded-lg p-4 mt-3">
                          ...
                          <p className="text-sm text-foreground">
                            {review.replies.find((reply) => reply.authorType === "staff")?.body ??
                        ...
                        <div className="bg-muted rounded-lg p-4 mt-3 space-y-3">
                          ...
                          <div className="flex gap-2">
                            <Button
                              size="sm"
                              onClick={() =>
                                replyMutation.mutate({ reviewId: review.id, body: replyText })
                              }
                      ...
                      {review.status === "pending" && (
                        <>
                          <Button
                            ...
                            onClick={() =>
                              moderateMutation.mutate({ reviewId: review.id, action: "approve" })
                            }
                          ...
                          <Button
                            ...
                            onClick={() =>
                              moderateMutation.mutate({ reviewId: review.id, action: "reject" })
                            }
                            disabled={moderateMutation.isPending}
                          >
