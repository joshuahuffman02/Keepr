  ...
  const runHealthChecks = async () => {
    ...
    try {
      ...
      results.push({
        ...
        latency,
        message: res.ok ? `Authenticated (${latency}ms)` : `Response in ${latency}ms`,
        lastChecked: new Date(),
      });
    } catch (err: unknown) {
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-sm text-muted-foreground">
                {checks.filter((c) => c.status === "healthy").length} of {checks.length} services
                healthy
              </div>
            </div>
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {checks.map((check) => (
          <div key={check.name} className="bg-muted rounded-lg border border-border p-4">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                {check.name === "API Server" && (
                  <Server className="h-5 w-5 text-muted-foreground" />
                )}
                {check.name === "Database" && (
                  <Database className="h-5 w-5 text-muted-foreground" />
                )}
                {check.name === "Auth Service" && (
                  <Activity className="h-5 w-5 text-muted-foreground" />
                )}
                {check.name === "Local Storage" && (
