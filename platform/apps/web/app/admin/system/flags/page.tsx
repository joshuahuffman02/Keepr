    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        {filtered.map((flag) => (
          <div
            key={flag.id}
            className={`bg-muted rounded-lg border p-4 transition-colors ${
              flag.enabled ? "border-status-success/30" : "border-border"
            }`}
          >
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-start gap-3">
                <div
                  className={`p-2 rounded-lg ${flag.enabled ? "bg-status-success/15" : "bg-muted"}`}
                >
                  <Flag
                    className={`h-4 w-4 ${flag.enabled ? "text-status-success" : "text-muted-foreground"}`}
                  />
                </div>
                <div>
