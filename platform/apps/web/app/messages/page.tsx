...
export default function MessagesPage() {
  ...
  const [composeSelectedGuest, setComposeSelectedGuest] = useState<{
    id: string;
    name: string;
    email?: string;
    phone?: string;
    reservations?: { id: string; siteName: string; arrivalDate: string; status: string }[];
  } | null>(null);
  const [composeSelectedReservation, setComposeSelectedReservation] = useState<{
    id: string;
    siteName: string;
    arrivalDate: string;
  } | null>(null);
  ...
  const activeFilterCount =
    (statusFilter !== "all" ? 1 : 0) + (dateRange.start ? 1 : 0) + (dateRange.end ? 1 : 0);
  ...
  const SLA_MINUTES =
    campground && "slaMinutes" in campground && typeof campground.slaMinutes === "number"
  ...
  const { data: searchedGuests = [] } = useQuery<Guest[]>({
    ...
    queryFn: () =>
      apiClient.getGuests(campground!.id, {
  ...
  useEffect(() => {
    if (
      activeTab === "team" &&
      !selectedInternalConversationId &&
  ...
  const selectedSmsConversation = smsConversations.find(
    (c) => c.conversationId === selectedSmsConversationId,
  );
    ...
    if (conv.unreadCount > 0) {
      try {
        ...
        queryClient.setQueryData(
          ["sms-conversations", campground?.id],
          (old: typeof smsConversations) => {
            if (!old) return old;
            return old.map((c) =>
  ...
  const handleSendSmsReply = async () => {
    ...
    setSendingSms(true);
    try {
      await apiClient.replySmsConversation(
        selectedSmsConversationId,
        campground.id,
        newSmsMessage.trim(),
      );
      setNewSmsMessage("");
  ...
  const handleComposeSubmit = async () => {
    ...
    const toAddress =
      composeType === "email" ? composeSelectedGuest.email : composeSelectedGuest.phone;
    if (!toAddress) {
      toast({
  ...
  const selectedInternalConversation = internalConversations.find(
    (c) => c.id === selectedInternalConversationId,
  );
  ...
  const handleSendInternalMessage = async () => {
    ...
    setSending(true);
    try {
      await apiClient.sendInternalMessage(
        selectedInternalConversationId,
        newInternalMessage.trim(),
      );
  ...
  const filteredConversations = useMemo(() => {
    ...
    return conversations.filter((conv) => {
      ...
      if (
        termLower &&
        !conv.guestName.toLowerCase().includes(termLower) &&
        !conv.siteName.toLowerCase().includes(termLower)
      ) {
        return false;
      }
      // Guest filter
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-muted-foreground hidden sm:inline">
              ...
              {overdueCount > 0 && (
                <span className="text-amber-600 ml-1">• {overdueCount} need reply</span>
              )}
            </span>
          </div>
              ...
              <CardContent className="pt-0">
                <div
                  className={cn(
                    ...
                    overdueCount > 0 ? "text-status-warning" : "text-status-success",
                  )}
                >
                  {overdueCount}
                </div>
                <div className="text-xs text-muted-foreground">
                  {overdueCount > 0
                    ? `Over ${SLA_MINUTES} min SLA`
                    : `Within ${SLA_MINUTES} min SLA`}
                </div>
              </CardContent>
            </button>
          </motion.div>
        ...
        <div className="md:hidden">
          <div className="rounded-2xl border bg-card shadow-sm p-2 flex flex-wrap gap-2">
            ...
            <Button
              size="sm"
              variant={
                activeTab === "guests" && guestFilter === "overdue" ? "destructive" : "outline"
              }
          ...
          <Card className="w-full lg:w-80 flex flex-col">
            ...
            >
              ...
              <div className="px-4 pb-3">
                <div className="relative">
                  ...
                  <Input
                    ...
                    title={
                      activeTab === "guests"
                        ? "Search by guest name or site number"
                        : "Search by channel name or team member"
                    }
                  />
                  {searchTerm && (
                    <button
                    ...
                    <div className="text-xs text-muted-foreground flex items-center gap-1.5">
                      ...
                      <div className="group relative">
                        ...
                        <div className="invisible group-hover:visible absolute left-0 top-5 z-50 w-64 p-2 bg-popover text-popover-foreground text-xs rounded-md border shadow-lg">
                          Service Level Agreement: Guest messages should be replied to within{" "}
                          {SLA_MINUTES} minutes. Overdue conversations are highlighted in amber.
                        </div>
                      </div>
              ...
              <div
                ...
                style={{
                  display: activeTab === "guests" ? "block" : "none",
                  flex: activeTab === "guests" ? 1 : "none",
                }}
              >
                    ...
                    {guestFilter === "overdue" ? (
                      <>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground mt-1">
                            No conversations need reply. Great work staying on top of guest
                            messages!
                          </p>
                        </div>
                      </>
                      ...
                      <>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground mt-1">
                            Try searching for a different guest or site
                          </p>
                        </div>
                      </>
                    ) : (
                      <>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground mt-1">
                            Guest conversations will appear here when they send messages
                          </p>
                        </div>
                      </>
                    )}
                            ...
                            <div className="flex items-start justify-between gap-2">
                              <div className="font-medium truncate flex items-center gap-2 text-foreground">
                                ...
                                {isOverdue && (
                                  <Badge
                                    variant="outline"
                                    className="bg-status-warning/15 text-status-warning border-status-warning/30 text-[10px] px-1.5 py-0"
                                  >
                                    SLA
                                  </Badge>
                                )}
                ...
                {loadingSmsConversations ? (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground mt-1">
                        SMS conversations will appear here when guests text your number
                      </p>
                    </div>
                  </motion.div>
                ) : (
                        ...
                        <DialogContent>
                                      ...
                                      <Checkbox
                                        ...
                                        onCheckedChange={(checked) => {
                                          ...
                                          if (checked)
                                            ...
                                            setSelectedParticipants(
                                              selectedParticipants.filter((id) => id !== uid),
                                            );
                                        }}
                                      />
                                      <Label htmlFor={`c-${member.user.id}`}>
                          ...
                          <DialogFooter>
                            <Button
                              ...
                              disabled={createConversationMutation.isPending || !newChannelName}
                            >
                              Create Channel
                            </Button>
                          </DialogFooter>
                      ...
                      {channels.map((conv) => (
                        <button
                          ...
                          className={`w-full text-left px-3 py-2 rounded-md flex items-center gap-2 transition-colors ${
                            selectedInternalConversationId === conv.id
                                ...
                                {staffMembers
                                  ...
                                  .map((member) => (
                                    <div key={member.id} className="flex items-center space-x-2">
                                      ...
                                      />
                                      <Label htmlFor={`dm-${member.user.id}`}>
                                        {member.user.firstName ?? ""} {member.user.lastName ?? ""}
                                      </Label>
                                    </div>
                                  ))}
                    ...
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        <CardTitle className="flex items-center gap-2">
                          ...
                          {selectedSmsConversation.guestName ||
                            selectedSmsConversation.phoneNumber ||
                            "Unknown"}
                        </CardTitle>
                        <CardDescription>
                          {selectedSmsConversation.phoneNumber} •{" "}
                          {selectedSmsConversation.messageCount} messages
                        </CardDescription>
                      </div>
                    </div>
                  </CardHeader>
                        ...
                        <AnimatePresence mode="popLayout">
                          {selectedSmsMessages.messages.map((msg, index) => {
                            ...
                            return (
                              ...
                              >
                                <div
                                  className={`max-w-[85%] sm:max-w-[65%] rounded-lg p-3 ${
                                ...
                                >
                                  ...
                                  {msg.reservation && (
                                    ...
                                    >
                                      ...
                                      Site {msg.reservation.site?.siteNumber || "—"} •{" "}
                                      {format(new Date(msg.reservation.arrivalDate), "MMM d")}
                                    </Link>
                                  )}
                    ...
                    <div className="space-y-4">
                      {internalMessages.length === 0 ? (
                        ...
                        internalMessages.map((msg) => (
                          <div
                            key={msg.id}
                            className={`flex flex-col gap-1 ${msg.senderId === session?.user?.id ? "items-end" : "items-start"}`}
                          >
                            <div className="flex items-center gap-2">
                            ...
                            <div
                              className={`p-3 rounded-lg max-w-[85%] sm:max-w-[80%] ${msg.senderId === session?.user?.id ? "bg-primary text-primary-foreground" : "bg-muted"}`}
                            >
                  ...
                  <div className="p-4 border-t">
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        disabled={sending || !newInternalMessage.trim()}
                      >
                        {sending ? (
                          <Loader2 className="h-4 w-4 animate-spin" />
                        ) : (
                ...
                <CardHeader className="border-b">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <CardDescription className="space-y-1">
                        ...
                        <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                          {selectedConversation.arrivalDate &&
                            selectedConversation.departureDate && (
                              <span className="flex items-center gap-1">
                                <Calendar className="h-3 w-3" />
                                {format(new Date(selectedConversation.arrivalDate), "MMM d")} -{" "}
                                {format(new Date(selectedConversation.departureDate), "MMM d")}
                              </span>
                            )}
                          ...
                          {selectedConversation.pets !== null && selectedConversation.pets > 0 && (
                            <span className="flex items-center gap-1">
                              ...
                              {selectedConversation.pets} pet
                              {selectedConversation.pets !== 1 ? "s" : ""}
                            </span>
                          )}
                        </div>
                      </CardDescription>
                    ...
                    <div className="flex items-center gap-2">
                      <Badge
                        variant={
                          selectedConversation.status === "checked_in" ? "default" : "secondary"
                        }
                      >
                      ...
                      <Button
                        ...
                        onClick={() =>
                          window.open(
                            `/reservations/${selectedConversation.reservationId}`,
                            "_blank",
                          )
                        }
                      >
                              ...
                              >
                                ...
                                >
                                  <div className="flex items-center gap-2 mb-1">
                                    <span
                                      className={`px-2 py-0.5 rounded-full text-[11px] font-semibold uppercase ${badgeClasses}`}
                                    >
                                      {isStaff ? "Staff" : "Guest"}
                                    </span>
                                  </div>
                            ...
                            <div className="space-y-2 text-sm">
                                ...
                                selectedConversation.departureDate && (
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span>
                                      ...
                                      -{" "}
                                      {format(
                                        new Date(selectedConversation.departureDate),
                                        "MMM d, yyyy",
                                      )}
                                    </span>
                                  </div>
                              ...
                              {(selectedConversation.adults || selectedConversation.children) && (
                                <div className="flex items-center gap-2">
                                  ...
                                  <span>
                                    {selectedConversation.adults || 0} Adult
                                    {(selectedConversation.adults || 0) !== 1 ? "s" : ""}
                                    {selectedConversation.children
                                      ? `, ${selectedConversation.children} Child${selectedConversation.children !== 1 ? "ren" : ""}`
                                      : ""}
                                  </span>
                                </div>
                              )}

                              {selectedConversation.pets !== null &&
                            ...
                            <div className="space-y-1">
                              ...
                              {selectedConversation.guestId && (
                                <Button
                                  ...
                                  className="w-full justify-start"
                                  onClick={() =>
                                    window.open(`/guests/${selectedConversation.guestId}`, "_blank")
                                  }
                                >
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  {composeType === "message" &&
                    "Sends to the guest's portal inbox (requires a reservation)"}
                  ...
                  <div className="relative">
                    ...
                    {searchedGuests.length === 0 && (
                      <div className="absolute top-full left-0 right-0 mt-1 border rounded-md bg-popover shadow-lg z-50 p-4 text-center text-sm text-muted-foreground">
                        {composeGuestSearch
                          ? `No guests found for "${composeGuestSearch}"`
                          : "No guests found"}
                      </div>
                    )}
                  </div>
              ...
              {composeType === "message" && composeSelectedGuest && (
                <div className="space-y-2">
                  ...
                  {composeSelectedGuest.reservations &&
                  composeSelectedGuest.reservations.length > 0 ? (
                    <div className="space-y-2">
                      {composeSelectedGuest.reservations.map((res) => (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground">
                              {res.arrivalDate
                                ? format(new Date(res.arrivalDate), "MMM d, yyyy")
                                : "No date"}{" "}
                              • {res.status}
                            </p>
                          </div>
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  placeholder={
                    composeType === "email"
                      ? "Write your email..."
                      : "Write your SMS (160 chars recommended)..."
                  }
                  value={composeBody}
