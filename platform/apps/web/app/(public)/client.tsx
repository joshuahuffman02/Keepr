...
export function HomeClient() {
  ...
  const updateAmenityParams = useCallback(
    (nextFilters: string[]) => {
      const params = new URLSearchParams(searchParams.toString());
  ...
  const heroFeatured = useMemo(() => {
    ...
    const withImages = internalCampgrounds.filter(
      (cg) => !cg.isExternal && (cg.heroImageUrl || (cg.photos?.length ?? 0) > 0),
    );
    ...
    const scoreByQuality = (
      a: (typeof internalCampgrounds)[number],
      b: (typeof internalCampgrounds)[number],
    ) => {
    ...
    let selected =
      [...prioritizedWorldClass].sort(scoreByQuality)[0] ?? [...worldClass].sort(scoreByQuality)[0];
  ...
  const { startDate, endDate } = hasMounted
  ...
  const allCampgrounds = useMemo(() => {
    ...
    if (activeCategory !== "all") {
      ...
      if (category && category.siteTypes.length > 0) {
        ...
        filtered = filtered.filter((cg) => {
          ...
          const amenitiesStr =
            ("amenities" in cg ? cg.amenities : [])?.join(" ").toLowerCase() || "";
          ...
          switch (activeCategory) {
            ...
            case "unique":
              return (
                combined.includes("yurt") ||
                combined.includes("treehouse") ||
                combined.includes("tiny") ||
                combined.includes("airstream")
              );
            default:
              return true;
          }
    ...
    if (amenityFilters.length > 0) {
      filtered = filtered.filter((cg) => {
        const cgAmenities =
          ("amenities" in cg ? cg.amenities : [])?.map((a: string) => a.toLowerCase()) || [];
                ...
                <p className="text-slate-600 mt-1">
                  {activeCategory === "events" ? (
                    <>
                      ...
                      {dateRangeFilter &&
                        ` (${DATE_RANGES.find((r) => r.value === dateRangeFilter)?.label})`}
                    </>
                  ) : (
                    <>
          ...
          <div className="flex flex-wrap gap-2">
            {activeCategory !== "events" &&
              COMMON_AMENITIES.slice(0, 8).map((amenity) => {
                ...
                const isSelected = amenityFilters.some(
                  (filter) => filter.toLowerCase() === amenityKey,
                );
                return (
                  <button
                    key={amenity.name}
          ...
          <>
            ...
            >
              {allCampgrounds.slice(0, displayCount).map((campground, index) => {
                const pricePerNight =
                  "pricePerNight" in campground ? campground.pricePerNight : undefined;
                const adaLevel =
                  "adaCertificationLevel" in campground
                    ? campground.adaCertificationLevel
                    : undefined;
                const resolvedAdaLevel = isAdaCertificationLevel(adaLevel) ? adaLevel : undefined;
                ...
                return (
                  ...
                  >
                    <CampgroundCard
                      ...
                      country={
                        "country" in campground ? campground.country || undefined : undefined
                      }
                      imageUrl={
                        "heroImageUrl" in campground
                          ? campground.heroImageUrl || undefined
                          : undefined
                      }
                      ...
                      onExplore={() =>
                        trackEvent("site_card_view", { campgroundId: campground.id, page: "/" })
                      }
                    />
                  </motion.div>
                );
          ...
          >
            ...
            <div className="space-y-3">
              ...
              <p className="text-lg text-slate-600 max-w-2xl mx-auto">
                Thousands of families have found their perfect getaway. Your next memory is just a
                few clicks away.
              </p>
            </div>
