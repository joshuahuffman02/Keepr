...
const isAmenityDisplay = (value: AmenityDisplay | null): value is AmenityDisplay => value !== null;
  ...
  return (
    <div className="inline-flex items-center gap-2 bg-status-warning/15 text-status-warning px-3 py-1 rounded-full border border-status-warning/30 text-sm font-semibold">
      <span className="flex items-center gap-1">
        <Star className="h-4 w-4 text-status-warning" /> {numeric.toFixed(1)}
      </span>
...
function PhotoGallery({
  ...
  useEffect(() => {
    ...
    trackEvent("image_viewed", {
      campgroundId,
      imageId: allPhotos[activeIndex],
      page: "campground_detail",
    });
      ...
      >
        ...
        <div className="absolute inset-0 flex flex-col justify-end p-6 md:p-10">
          <div className="max-w-3xl space-y-3">
            ...
            {/* Tagline */}
            {tagline && (
              <p className="text-lg md:text-xl text-white/90 font-medium max-w-2xl">{tagline}</p>
            )}
          ...
          {allPhotos.map((_, i) => (
            <button
              ...
              className={`w-3 h-3 rounded-full transition-all ${
                i === activeIndex ? "bg-white scale-110" : "bg-white/50 hover:bg-white/75"
              }`}
            />
          ))}
        </div>
      ...
      <div className="p-5">
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          ...
          {siteClass.hookupsWater && (
            <span className="text-xs bg-status-info/15 text-status-info px-2 py-1 rounded inline-flex items-center gap-1">
              <Droplet className="h-3 w-3" /> Water
            </span>
          )}
          {siteClass.hookupsSewer && (
            <span className="text-xs bg-status-success/15 text-status-success px-2 py-1 rounded inline-flex items-center gap-1">
              <Plug className="h-3 w-3" /> Sewer
            </span>
          )}
          {siteClass.petFriendly && (
            <span className="text-xs bg-status-warning/15 text-status-warning px-2 py-1 rounded inline-flex items-center gap-1">
              <Dog className="h-3 w-3" /> Pets OK
            </span>
          )}
          ...
          {displayAmenities.map((amenity) => (
            <span
              key={amenity.id}
              className="text-xs bg-status-success/15 text-status-success px-2 py-1 rounded inline-flex items-center gap-1"
            >
...
export function CampgroundDetailClient({
  ...
  const filteredReviews = useMemo(() => {
    ...
    const byTag = tagFilter
      ? byPhotos.filter((r) =>
          (r.tags || []).map((t) => t.toLowerCase()).includes(tagFilter.toLowerCase()),
        )
      : byPhotos;
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-slate-600">
          The campground you're looking for doesn't exist or isn't available.
        </p>
        <Link
          href="/"
      ...
      <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
        ...
        <div className="max-w-7xl mx-auto px-4 pb-12">
            ...
            <div className="flex flex-col items-end gap-3">
              ...
              {campground.phone && (
                <a
                  ...
                  className="text-slate-600 hover:text-emerald-600 inline-flex items-center gap-1"
                >
            ...
            <div className="mb-8 flex items-start gap-3 p-4 bg-status-warning/15 border border-status-warning/30 rounded-xl">
              ...
              <div>
                <p className="font-semibold text-status-warning">
                  Discovery listing — booking handled off-platform
                </p>
                <p className="text-sm text-status-warning">
                  Booking is disabled here.{" "}
                  {externalHref
                    ? "Use the website link to reserve directly."
                    : "Please contact the campground to book."}
                </p>
              </div>
            </div>
          )}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
              {campground.checkInTime && (
                <div>
                  ...
                  <div className="font-semibold text-slate-800">
                    {formatTime(campground.checkInTime)}
                  </div>
                </div>
              )}
              {campground.checkOutTime && (
                <div>
                  ...
                  <div className="font-semibold text-slate-800">
                    {formatTime(campground.checkOutTime)}
                  </div>
                </div>
              )}
              ...
              {campground.seasonEnd && (
                <div>
                  <div className="flex justify-center mb-1">
                    <TreeDeciduous className="h-6 w-6 text-amber-500" />
                  </div>
          ...
          <section className="mb-12">
              ...
              {campground.reviewScore ? (
                <div className="flex items-center gap-2 bg-status-warning/15 text-status-warning px-3 py-2 rounded-full border border-status-warning/30 text-sm font-semibold">
                  ...
                  <span className="text-status-warning text-xs">
                    {campground.reviewCount ?? 0} reviews
                  </span>
                </div>
              ) : null}
            </div>
                        ...
                        {review.replies.map((reply) => (
                          <div key={reply.id} className="text-xs text-slate-600">
                            <span className="font-semibold text-slate-800">
                              {reply.authorType === "staff" ? "Host" : "Guest"}:
                            </span>{" "}
                            {reply.body}
                          </div>
                        ))}
            ...
            {reviewsQuery.error && (
              <div className="text-sm text-status-error">Failed to load reviews.</div>
            )}
          </section>
            ...
            <section className="mb-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {campground.events.map((event, index) => {
                  ...
                  return <EventCard key={eventId ?? `${index}`} event={event} slug={slug} />;
                })}
              </div>
            </section>
          )}
            ...
            <section className="mb-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {campground.promotions.map((promo) => (
                  <div
                    key={promo.id}
                    className="bg-gradient-to-br from-rose-50 to-orange-50 rounded-xl border-2 border-rose-100 p-5 relative overflow-hidden group hover:shadow-lg transition-all hover:scale-[1.02]"
                  >
                    ...
                    <div className="mb-4">
                      ...
                      <p className="text-slate-600 text-sm mt-1">
                        {promo.description || "Special offer for our guests!"}
                      </p>
                    </div>
