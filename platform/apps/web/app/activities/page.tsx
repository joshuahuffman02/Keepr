        ...
        <div className="absolute top-3 right-3 flex gap-1.5">
          ...
          <Badge
            ...
            className={cn(
              ...
              activity.isActive ? "bg-emerald-500 hover:bg-emerald-600" : "bg-slate-400",
            )}
          >
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl p-8 text-center motion-safe:animate-in motion-safe:zoom-in-95 motion-safe:fade-in duration-300 max-w-md mx-4">
        ...
        <h3 className="text-xl font-bold text-slate-900 mb-2">Activity Created!</h3>
        <p className="text-slate-600 mb-4">
          <span className="font-medium text-emerald-600">{activityName}</span> is ready for guests
          to enjoy
        </p>
    ...
    <div className="col-span-full">
      <div className="relative overflow-hidden rounded-2xl border-2 border-dashed border-slate-200 bg-muted p-12 text-center">
        ...
        <div className="relative">
          ...
          <h3 className="text-2xl font-bold text-slate-900 mb-3">Create Your First Activity</h3>
          <p className="text-slate-600 max-w-md mx-auto mb-8">
            Activities make your campground memorable! Add guided tours, yoga sessions, kayak
            rentals, or campfire nights for your guests to enjoy.
          </p>
          ...
          <div className="mt-10 flex flex-wrap justify-center gap-3">
            {["Morning Yoga", "Kayak Rental", "Campfire Night", "Nature Hike", "Fishing Trip"].map(
              (name) => (
                <span
                  key={name}
...
export default function ActivitiesPage() {
  ...
  const [selectedActivityForSessions, setSelectedActivityForSessions] = useState<string | null>(
    null,
  );
  ...
  const { data: sessions, refetch: refetchSessions } = useQuery({
    ...
    queryFn: () =>
      selectedActivityForSessions
        ? apiClient.getSessions(selectedActivityForSessions)
        : Promise.resolve([]),
  ...
  const handleImageUpload = async (file: File) => {
    ...
    try {
      ...
      toast({
        title: "Upload failed",
        description: getErrorMessage(err, "Upload failed"),
        variant: "destructive",
      });
    } finally {
      setUploading(false);
  ...
  return (
    <DashboardShell>
      ...
      <Dialog
        open={!!selectedActivityForSessions}
        onOpenChange={(open) => !open && setSelectedActivityForSessions(null)}
      >
          ...
          <DialogHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <DialogDescription>
                  Schedule upcoming sessions for guests to book.
                </DialogDescription>
              </div>
              {selectedActivity && (
                <Button
          ...
          <div className="grid gap-6 py-4">
            <div className="border rounded-lg p-4 bg-status-success/10 space-y-4">
              ...
              <Button
                ...
                disabled={
                  createSessionMutation.isPending || !newSession.startTime || !newSession.endTime
                }
                ...
                {sessions?.map((session) => (
                  ...
                  >
                    ...
                    <Badge
                      ...
                      className={
                        session.status === "open" ? "bg-status-success/15 text-status-success" : ""
                      }
                    >
                      {session.status}
                    </Badge>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <button
                  type="button"
                  onClick={() =>
                    setNewActivity({ ...newActivity, schedulingMode: "open_availability" })
                  }
                  className={cn(
                    "p-4 rounded-lg border-2 text-left transition-all",
                ...
                <div className="grid grid-cols-2 gap-4 pt-2 border-t border-slate-200">
                  ...
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setNewActivity({ ...newActivity, maxConcurrent: e.target.value })
                      }
                      placeholder="3"
                    />
                  </div>
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-1">Create memorable experiences for your guests</p>
          </div>
          {viewMode !== "bundles" && (
            <Button
              onClick={() =>
                viewMode === "cards" ? setIsCreateOpen(true) : setIsCreateEventOpen(true)
              }
              className="bg-status-success text-white shadow-sm transition-all duration-200 hover:shadow-md hover:-translate-y-0.5"
            >
              <Plus className="h-4 w-4 mr-2" />
        ...
        >
          {campgroundId && activities && (
            <BundlesManager campgroundId={campgroundId} activities={activities} />
          )}
        </div>
      </div>
    </DashboardShell>
