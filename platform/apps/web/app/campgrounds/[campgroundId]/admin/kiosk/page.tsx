...
export default function KioskDevicesPage() {
  ...
  const revokeMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setMessage({
        type: "success",
        text: "Device paused successfully. Guests cannot check in until re-enabled.",
      });
    },
    onError: (err: unknown) => {
  ...
  const getLastSeenStatus = (lastSeenAt: string | null | undefined) => {
    if (!lastSeenAt)
      return { label: "Never connected", color: "text-muted-foreground", icon: WifiOff };
    const diff = Date.now() - new Date(lastSeenAt).getTime();
    const hours = diff / (1000 * 60 * 60);
    if (hours < 1) return { label: "Online", color: "text-emerald-600", icon: Wifi };
    if (hours < 24)
      return { label: `${Math.floor(hours)}h ago`, color: "text-amber-600", icon: Wifi };
              ...
              {!pairingCode ? (
                <div className="flex flex-col items-center gap-4 py-6">
                  ...
                  <p className="text-sm text-muted-foreground text-center max-w-md">
                    Kiosks let guests check themselves in without waiting for staff. Perfect for
                    after-hours arrivals and busy check-in times.
                  </p>
                  <Button
              ...
              {devicesQuery.isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-sm text-muted-foreground max-w-sm">
                    Generate a pairing code above to connect your first kiosk. Guests will be able
                    to check in themselves 24/7.
                  </p>
                </div>
              ) : (
                        ...
                        <div className="flex items-center gap-4 min-w-0">
                          <div
                            className={cn(
                              ...
                              device.status === "active" ? "bg-status-success/15" : "bg-muted",
                            )}
                          >
                            <Tablet
                              className={cn(
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-3 mt-1">
                              <span
                                className={cn("flex items-center gap-1 text-xs", lastSeen.color)}
                              >
      ...
      <AlertDialog open={!!deleteConfirm} onOpenChange={(open) => !open && setDeleteConfirm(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to remove "{deleteConfirm?.name}" permanently? This action
              cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
