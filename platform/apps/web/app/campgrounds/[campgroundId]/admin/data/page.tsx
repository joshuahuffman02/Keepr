...
const getString = (value: unknown): string | undefined => (isString(value) ? value : undefined);

const getNumber = (value: unknown): number | undefined => (isNumber(value) ? value : undefined);
...
const parseImportResult = (
  value: unknown,
): { validCount: number; errorCount: number; jobId?: string } => {
  if (!isRecord(value)) {
...
export default function DataOperationsPage() {
    ...
    try {
      ...
      if (importFormat === "json") {
        try {
          ...
          setImportMessage({
            type: "error",
            text: "Invalid JSON file. Please check the format and try again.",
          });
          setImporting(false);
          return;
        }
      ...
      setImportMessage({
        type: "error",
        text: getErrorMessage(err) || "Import failed. Please try again.",
      });
    } finally {
      setImporting(false);
    }
  ...
  const handleDownloadTemplate = async () => {
    ...
    try {
      ...
      setImportMessage({
        type: "success",
        text: "Template downloaded! Fill it out and import above.",
      });
    } catch (err) {
      setImportMessage({
        type: "error",
        text: getErrorMessage(err) || "Failed to download template",
      });
    } finally {
    ...
    try {
      ...
      if (exportFormat === "csv" && exportResult.csv) {
        ...
        downloadBlob(
          ...
          `reservations-${campgroundId}.json`,
          "application/json",
        );
      }
      setExportMessage({
  ...
  const handleQueueExportJob = async () => {
    ...
    try {
      ...
      setExportMessage({
        type: "error",
        text: getErrorMessage(err) || "Failed to queue export job",
      });
    } finally {
      setExporting(false);
    }
              ...
              <div className="grid gap-6 md:grid-cols-2">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <Label htmlFor="include-pii" className="text-foreground">
                        Include personal info
                      </Label>
                      <p className="text-xs text-muted-foreground">Names, emails, phone numbers</p>
                    </div>
                  </div>
                  ...
                  {includePii && (
                    ...
                    >
                      ...
                      <span>
                        Handle exported PII data securely and in compliance with privacy
                        regulations.
                      </span>
                    </motion.div>
                  )}
                  ...
                  <div className="rounded-lg border border-border bg-muted/30 p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          First page of results (up to 1,000 rows)
                        </p>
                      </div>
                      <Button
