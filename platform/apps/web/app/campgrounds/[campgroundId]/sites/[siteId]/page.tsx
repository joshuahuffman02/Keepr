...
export default function SiteDetailPage() {
  ...
  if (siteQuery.isLoading) {
    return (
      <DashboardShell>
        <div className="flex h-80 items-center justify-center text-slate-600">Loading site…</div>
      </DashboardShell>
    );
  }
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-slate-900">{site.name || `Site #${site.siteNumber}`}</h1>
              <div className="text-sm text-slate-500">#{site.siteNumber} • {site.siteType}</div>
            </div>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Card>
            ...
            <CardContent className="grid grid-cols-2 gap-3 text-sm">
              <div>
                <div className="text-xs text-slate-500">Max occupancy</div>
              ...
              <div>
                <div className="text-xs text-slate-500">Sewer</div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              {statusQuery.isLoading && <div className="text-xs text-slate-500">Checking…</div>}
              ...
              {status && (
                <div className="rounded border border-slate-200 px-3 py-2">
                  <div className="flex items-center justify-between">
                    ...
                    {status.statusDetail && <span className="text-xs text-slate-500">{status.statusDetail}</span>}
                  </div>
                  <div className="text-xs text-slate-500">
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              {reservationsQuery.isLoading && <div className="text-xs text-slate-500">Loading…</div>}
              ...
              {upcomingReservations.length === 0 && !reservationsQuery.isLoading && (
                <div className="text-xs text-slate-500">No upcoming stays on this site.</div>
              )}
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              {maintenanceQuery.isLoading && <div className="text-xs text-slate-500">Loading…</div>}
              ...
              {openMaintenance.length === 0 && !maintenanceQuery.isLoading && (
                <div className="text-xs text-slate-500">No open tickets for this site.</div>
              )}
              {openMaintenance.map((m) => (
                <div key={m.id} className="rounded border border-slate-200 px-3 py-2">
                  ...
                  {m.dueDate && <div className="text-xs text-slate-500">Due {m.dueDate}</div>}
                </div>
              ))}
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              {blackoutQuery.isLoading && <div className="text-xs text-slate-500">Loading…</div>}
              ...
              {relevantBlackouts.length === 0 && !blackoutQuery.isLoading && (
                <div className="text-xs text-slate-500">No blackout dates affecting this site.</div>
              )}
              {relevantBlackouts.map((b) => (
                <div key={b.id} className="rounded border border-slate-200 px-3 py-2">
