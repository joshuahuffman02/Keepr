...
export default function BillingSettingsPage() {
  ...
  const [bulkRows, setBulkRows] = useState<
    Array<{ meterId: string; readingValue: string; readAt: string }>
  ...
  const meteredSites = useMemo(() => {
    const meteredClassIds = new Set(
      siteClasses.filter((sc) => sc.meteredEnabled).map((sc) => sc.id),
    );

    return sites.filter((site) => site.siteClassId && meteredClassIds.has(site.siteClassId));
  }, [sites, siteClasses]);
  ...
  const handleCreateMeter = async () => {
    ...
    try {
      ...
      setMeterMessage({
        type: "error",
        text: getErrorMessage(err, "Failed to create meter. Please try again."),
      });
    } finally {
  ...
  const handleAddRead = async () => {
    ...
    setReadMessage(null);
    try {
      await apiClient.addUtilityMeterRead(
        readForm.meterId,
        {
      ...
      setReadMessage({
        type: "success",
        text: "Reading saved! Usage will be calculated on next billing cycle.",
      });
      setReadForm({ meterId: readForm.meterId, readingValue: "", readAt: "", note: "" });
    } catch (err: unknown) {
      setReadMessage({
        type: "error",
        text: getErrorMessage(err, "Failed to save reading. Please check the value and try again."),
      });
    } finally {
      setSavingRead(false);
    }
  ...
  const handleBulkImport = async () => {
    ...
    if (validRows.length === 0) {
      setImportMessage({
        type: "error",
        text: "Please fill in at least one row with meter and reading value.",
      });
      return;
    }
    ...
    try {
      ...
      setImportMessage({
        type: "success",
        text: `Imported ${validRows.length} reading${validRows.length > 1 ? "s" : ""} successfully!`,
      });
      ...
      setImportMessage({
        type: "error",
        text: getErrorMessage(err, "Failed to import readings. Please check your data."),
      });
    } finally {
  ...
  const handleRunLateFees = async () => {
    ...
    try {
      ...
      setLateFeeMessage({
        type: "success",
        text: "Late fee processing started. Overdue invoices will be updated shortly.",
      });
    } catch (err: unknown) {
      setLateFeeMessage({
        type: "error",
        text: getErrorMessage(err, "Failed to run late fees. Please try again."),
      });
    } finally {
              ...
              <CardContent className="space-y-6">
                ...
                {meteredSites.length > 0 && (
                  <div className="rounded-lg border border-border bg-muted/30 p-4">
                    ...
                    <div className="grid gap-4 sm:grid-cols-4">
                          ...
                          <Tooltip>
                            ...
                            <TooltipContent>
                              <p className="max-w-xs">
                                Only sites with metered billing enabled appear here. Enable it in
                                Site Class settings.
                              </p>
                            </TooltipContent>
                          </Tooltip>
                          ...
                          <SelectContent>
                            {availableSitesForMeter.length === 0 ? (
                              <div className="px-2 py-3 text-sm text-muted-foreground text-center">
                                All metered sites already have{" "}
                                {(meterTypeConfig[meterForm.type]?.label ?? "Meter").toLowerCase()}{" "}
                                meters
                              </div>
                            ) : (
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="meter-serial" className="text-foreground">
                          Serial Number
                        </Label>
                        <Input
                          id="meter-serial"
                          ...
                          onChange={(e) =>
                            setMeterForm({ ...meterForm, serialNumber: e.target.value })
                          }
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    >
                      <RefreshCw
                        className={cn("h-4 w-4", metersQuery.isFetching && "animate-spin")}
                      />
                    </Button>
                  </div>
                      ...
                      >
                        ...
                        {meteredSites.length > 0 && (
                          <p className="text-xs text-muted-foreground mt-3 flex items-center justify-center gap-1">
                            ...
                            {meteredSites.length} site{meteredSites.length !== 1 ? "s" : ""} ready
                            for meters
                          </p>
                        )}
                      </motion.div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid gap-4 sm:grid-cols-5">
                        <div className="space-y-2 sm:col-span-2">
                          <Label htmlFor="reading-meter" className="text-foreground">
                            Meter
                          </Label>
                          <Select
                            value={readForm.meterId}
                            onValueChange={(v) => setReadForm({ ...readForm, meterId: v })}
                          >
                        ...
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setReadForm({ ...readForm, readingValue: e.target.value })
                            }
                            placeholder="1234.56"
                          />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="reading-date" className="text-foreground">
                            Date
                          </Label>
                          <Input
                            id="reading-date"
                      ...
                      <CollapsibleContent className="pt-4">
                        ...
                        <div className="rounded-lg border border-border overflow-hidden">
                          <table
                            ...
                            role="table"
                            aria-label="Bulk meter readings entry"
                          >
                            <thead>
                                ...
                                <tr key={index} className="border-b border-border last:border-0">
                                  <td className="py-2 px-3">
                                    ...
                                    >
                                      <SelectTrigger
                                        className="h-9"
                                        aria-label={`Select meter for row ${index + 1}`}
                                      >
                                      ...
                                      <SelectContent>
                                        {meters.map((m) => {
                                          const config =
                                            meterTypeConfig[m.type] ?? meterTypeConfig.power;
                                          const Icon = config.icon;
                                          const site = sites.find((s) => s.id === m.siteId);
                                          return (
                                            <SelectItem key={m.id} value={m.id}>
                                  ...
                                  <td className="py-2 px-3">
                                    <Input
                                      ...
                                      onChange={(e) =>
                                        updateBulkRow(index, "readingValue", e.target.value)
                                      }
                                      placeholder="0.00"
                                  ...
                                  <td className="py-2 px-3">
                                    <Input
                                      ...
                                      value={row.readAt}
                                      onChange={(e) =>
                                        updateBulkRow(index, "readAt", e.target.value)
                                      }
                        ...
                        <div className="flex items-center justify-between mt-3">
                          ...
                          <Button
                            ...
                            disabled={
                              importingBulk || !bulkRows.some((r) => r.meterId && r.readingValue)
                            }
                          ...
                          >
                            {importingBulk ? (
                              ...
                              <>
                                ...
                                {bulkRows.filter((r) => r.meterId && r.readingValue).length || ""}{" "}
                                Readings
                              </>
                            )}
                          </Button>
                ...
                <AnimatePresence>
                  {lateFeeMessage && (
                    <MessageBanner
                      ...
                      onDismiss={() => setLateFeeMessage(null)}
                    />
                  )}
                </AnimatePresence>
                    ...
                    <table className="w-full text-sm" role="table" aria-label="Invoice results">
                      <thead>
                        <tr className="border-b border-border bg-muted/50">
                          ...
                          <th
                            scope="col"
                            className="text-right py-3 px-3 font-medium text-muted-foreground"
                          >
                            Total
                          </th>
                          <th
