...
import type {
  ...
  StoreLocation as StoreLocationModel,
} from "@keepr/shared";
...
const posApi = {
  ...
  getProductCategories: (campgroundId: string): Promise<Category[]> =>
    apiClient.getProductCategories(campgroundId),
  createStoreOrder: (
    campgroundId: string,
    payload: StoreOrderPayload,
    headers?: Record<string, string>,
  ) => apiClient.createStoreOrder(campgroundId, payload, headers),
  getLocations: (campgroundId: string): Promise<StoreLocation[]> =>
...
function RefundExchangeDialog({
  ...
  const selectedLineItems =
    activeOrder?.items?.filter((item: StoreOrderItem) => selectedItems[item.id]) ?? [];
  const computedAmount = selectedLineItems.reduce(
    (sum: number, item: StoreOrderItem) => sum + (item.totalCents ?? 0),
    0,
  );
  const amountCents = overrideAmount
    ? centsFromInput(overrideAmount, computedAmount)
    : computedAmount;
    ...
    >
      <DialogContent className="sm:max-w-[560px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Select a recent order, choose items, and capture a note.
          </DialogDescription>
        </DialogHeader>
              ...
              <SelectContent>
                ...
                {orders.map((order) => (
                  <SelectItem key={order.id} value={order.id}>
                    #{order.id.slice(0, 8)} • ${(order.totalCents / 100).toFixed(2)} •{" "}
                        ...
                        <div className="flex items-center gap-2">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              setSelectedItems((prev) => ({ ...prev, [item.id]: Boolean(checked) }))
                            }
                          />
...
export default function POSPage() {
  ...
  const flushQueue = async () => {
    ...
    for (const item of items) {
      ...
      try {
        const headers: Record<string, string> = item.idempotencyKey
  ...
  const addToCart = (product: Product) => {
    ...
    setCart((prev) => {
      ...
      if (existing) {
        ...
        setTimeout(() => {
          setCart((current) =>
            current.map((item) => (item.id === product.id ? { ...item, justAdded: false } : item)),
          );
        }, 600);
        return updated;
      }
  ...
  const handleCheckoutSuccess = (order: StoreOrder) => {
    ...
    const orderTotal =
      order?.totalCents ?? cart.reduce((sum, item) => sum + item.priceCents * item.qty, 0);
  ...
  const filteredProducts = products.filter((p) => {
    const matchesCategory =
      !selectedCategory || selectedCategory === "all" || p.categoryId === selectedCategory;
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-3">
              ...
              {locations.length > 1 && (
                <Select
                  ...
                  onValueChange={(value) =>
                    setSelectedLocationId(value === EMPTY_SELECT_VALUE ? null : value)
                  }
                >
                  <SelectTrigger
                    className="h-9 w-[200px] text-sm font-medium"
                    aria-label="Location"
                  >
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Button asChild size="sm" variant="outline" className="hidden lg:flex">
                <Link
                  href="/store"
                  title="Manage products, categories, add-ons, taxes, hours, channel allotments"
                >
                  Manage inventory
                </Link>
              </Button>
        ...
        <div className="hidden md:block border-t border-border bg-muted/40 px-6 pb-6">
          <div className="flex items-center justify-between py-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Click an order to process a refund or exchange.
              </p>
            </div>
            <Button
              size="sm"
          ...
          {ordersLoading ? (
            ...
            <div className="rounded-lg border-2 border-dashed border-border bg-muted p-8 text-center">
              ...
              <p className="text-xs text-muted-foreground mb-4">
                Start selling by adding products to your cart
              </p>
              {cart.length > 0 ? (
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      {order.siteNumber && (
                        <div className="text-xs text-muted-foreground">Site {order.siteNumber}</div>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                  ...
                  <div className="mt-3 rounded-md border border-border bg-muted px-3 py-2">
                    ...
                    {order.adjustments && order.adjustments.length > 0 ? (
                      <div className="space-y-1">
                        {order.adjustments
                          .slice(0, 3)
                          .map((adj: StoreOrder["adjustments"][number]) => (
                            <div
                              key={adj.id}
      ...
      {showSuccessCelebration && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 motion-safe:animate-in motion-safe:fade-in motion-safe:duration-200">
          <div className="bg-card rounded-2xl shadow-2xl p-8 text-center max-w-sm mx-4 motion-safe:animate-in motion-safe:zoom-in-95 motion-safe:duration-300">
            ...
            <p className="text-muted-foreground text-sm">Payment processed successfully</p>
