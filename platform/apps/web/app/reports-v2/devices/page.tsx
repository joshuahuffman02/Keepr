        ...
        >
          ...
          {campgroundId && (
            <>
              ...
              >
                {isLoading && (
                  <div className="text-sm text-muted-foreground">Loading device analytics...</div>
                )}
                {error && (
                  <div className="text-sm text-status-error">Unable to load device analytics.</div>
                )}
                {!isLoading && !error && (
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                    {(data?.devices || []).map((device) => {
                      ...
                      const share =
                        totals.sessions > 0
                          ? ((device.sessions / totals.sessions) * 100).toFixed(1)
                          : "0";
                      return (
                        <div
                        ...
                        >
                          ...
                          <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
                            ...
                            <div>
                              ...
                              <div className="text-lg font-semibold text-foreground">
                                {(device.conversionRate * 100).toFixed(1)}%
                              </div>
                            </div>
                          </div>
                        </div>
