...
import { ArrowDownRight, ArrowUpRight, BedDouble, CalendarDays, DollarSign, Users, Wrench } from "lucide-react";
...
export function DailySummaryReport({ campgroundId, dateRange }: DailySummaryReportProps) {
    ...
    const summary = useMemo(() => {
        ...
        const totalSiteDays = sites.length * ((end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24));
        ...
        const revenueEntries = ledgerEntries.filter(e => {
            ...
            return isWithinInterval(d, { start, end }) && (e.direction === 'credit' || (e.description?.toLowerCase().includes('charge')));
        });
        ...
        const closedTickets = tickets.filter(t => {
            const d = t.resolvedAt ? new Date(t.resolvedAt) : new Date(t.updatedAt || t.createdAt || new Date());
