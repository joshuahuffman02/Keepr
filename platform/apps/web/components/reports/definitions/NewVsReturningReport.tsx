...
export function NewVsReturningReport({ campgroundId, dateRange }: NewVsReturningReportProps) {
  ...
  const reportData = useMemo(() => {
    ...
    reservations.forEach((r) => {
      ...
      const isReturning =
        (guestRepeatStays[guestId] || 0) > 0 || (guestReservationCounts[guestId] || 0) > 1;
          ...
          <div className="p-6 bg-purple-50 rounded-xl border border-purple-100">
            ...
            <div className="mt-2 flex items-baseline gap-2">
              ...
              <span className="text-sm text-purple-700">
                (
                {reportData.total > 0
                  ? ((reportData.newCount / reportData.total) * 100).toFixed(1)
                  : 0}
                %)
              </span>
            </div>
          ...
          <div className="p-6 bg-blue-50 rounded-xl border border-blue-100">
            ...
            <div className="mt-2 flex items-baseline gap-2">
              ...
              <span className="text-sm text-blue-700">
                (
                {reportData.total > 0
                  ? ((reportData.returningCount / reportData.total) * 100).toFixed(1)
                  : 0}
                %)
              </span>
            </div>
