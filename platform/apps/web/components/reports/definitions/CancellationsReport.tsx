  ...
  const cancellations = useMemo(() => {
    ...
    return reservations
      .filter((r) => {
        ...
        return r.status === "cancelled" && arrivalDate >= start && arrivalDate <= end;
      })
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {cancellations.length} cancelled bookings for arrival between {dateRange.start} and{" "}
            {dateRange.end}
          </p>
        </div>
        <div className="text-right">
          ...
          <p className="text-xl font-bold text-rose-600">
            {formatCurrencyLocal(totalLostRevenue / 100)}
          </p>
        </div>
      </div>
                ...
                cancellations.map((r) => (
                  <tr key={r.id} className="hover:bg-muted group transition-colors">
                    ...
                    <td className="px-4 py-3 text-muted-foreground">
                      {format(new Date(r.arrivalDate), "MMM d, yyyy")}
                    </td>
                    <td className="px-4 py-3 text-muted-foreground">
                      {Math.ceil(
