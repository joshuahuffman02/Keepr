...
const PRODUCT_COLUMNS = [
  { key: "name", label: "Name", required: true, example: "Firewood Bundle" },
  {
    ...
    label: "Description",
    required: false,
    example: "Premium oak firewood, 10 pieces",
  },
  ...
  {
    key: "trackInventory",
    label: "Track Inventory (true/false)",
    required: false,
    example: "true",
  },
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      rows[0].forEach((header, idx) => {
        ...
        const matchedCol = PRODUCT_COLUMNS.find(
          (c) =>
            c.label.toLowerCase() === normalizedHeader || c.key.toLowerCase() === normalizedHeader,
        );
        if (matchedCol) {
  ...
  const runImport = async () => {
    ...
    for (let i = 0; i < rawData.length; i++) {
      ...
      try {
        ...
        const channelInventoryMode = product.channelInventoryMode === "split" ? "split" : "shared";
          ...
          {importStep === "upload" && (
            <div className="py-8 text-center space-y-4">
              ...
              <p className="text-xs text-muted-foreground">
                Need a template?{" "}
                <button onClick={downloadTemplate} className="text-emerald-600 underline">
                  Download here
                </button>
              </p>
            </div>
          ...
          {importStep === "preview" && (
            <div className="space-y-4">
                    ...
                    {previewData.map((row) => (
                      <tr key={row._rowNum}>
                        ...
                        <td className="px-3 py-2">
                          {row.priceCents ? (
                            `$${(parseInt(row.priceCents) / 100).toFixed(2)}`
                          ) : (
              ...
              <div className="flex justify-end gap-2 pt-4">
                ...
                <Button onClick={runImport}>Import {rawData.length} Products</Button>
              </div>
            </div>
          )}
