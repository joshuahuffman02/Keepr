...
export function CampgroundCard({
  ...
  const handleToggleFavorite = useCallback(
    (e: React.MouseEvent) => {
      ...
      setIsFavorited((prev) => {
        ...
        const saved = readSaved();
        const nextList = next ? Array.from(new Set([...saved, id])) : saved.filter((x) => x !== id);
        writeSaved(nextList);
        return next;
      });
    },
  ...
  const allImages = imageUrl ? [imageUrl, ...photos.filter((p) => p !== imageUrl)] : photos;
  ...
  const gradients = [
    ...
    "from-keepr-charcoal to-keepr-evergreen",
    "from-keepr-charcoal to-keepr-clay",
    "from-keepr-evergreen-light to-keepr-clay-light",
    "from-keepr-clay to-keepr-evergreen",
  ];
    ...
    <div className="group relative bg-card rounded-2xl overflow-hidden shadow-lg shadow-slate-900/5 hover:shadow-2xl hover:shadow-slate-900/10 transition-all duration-300 transform hover:-translate-y-1">
      ...
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        >
          ...
          >
            <path
              ...
              strokeWidth={1.5}
              d="m4 20 8-14 8 14M2 20h20M9 15h6"
            />
          </svg>
        </div>
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-2">
          ...
          {npsBadge && (
            <div
              className={`px-3 py-1.5 text-white text-xs font-bold rounded-full shadow-lg flex items-center gap-1.5 ${
                npsBadge.type === "top-campground"
                  ? "bg-gradient-to-r from-keepr-clay to-keepr-clay-light"
                  : npsBadge.type === "rising-star"
                    ? "bg-gradient-to-r from-keepr-evergreen to-keepr-evergreen-light"
                    : npsBadge.type === "world-class"
                      ? "bg-gradient-to-r from-keepr-charcoal to-keepr-evergreen"
                      : npsBadge.type === "top-1"
                        ? "bg-gradient-to-r from-keepr-clay-light to-keepr-clay"
                        : npsBadge.type === "top-5"
                          ? "bg-gradient-to-r from-keepr-evergreen-light to-keepr-clay-light"
          ...
          {pastAwards.length > 0 && (
            <div className="px-3 py-1.5 bg-gradient-to-r from-keepr-clay to-keepr-clay-light text-white text-xs font-bold rounded-full shadow-lg flex items-center gap-1.5">
              <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20">
                <path
                  fillRule="evenodd"
          ...
          {isInternal && !npsBadge && !isExternal && (
            <div className="px-3 py-1.5 bg-gradient-to-r from-keepr-evergreen to-keepr-evergreen-light text-white text-xs font-bold rounded-full shadow-lg flex items-center gap-1.5">
        ...
        >
          <motion.div
            animate={isHeartAnimating ? { scale: [1, 1.3, 0.9, 1.1, 1] } : {}}
        ...
        >
          {isExternal ? (
            ...
            <>
              <Sparkles className="h-4 w-4 text-keepr-evergreen" />
      ...
      <div className={compact ? "p-3" : "p-5"}>
        ...
        {hasRating && (
          <div className={`flex items-center gap-2 ${compact ? "mb-1" : "mb-2"} text-sm flex-wrap`}>
            <div className="flex items-center gap-1">
              <svg className="w-4 h-4 text-keepr-clay" fill="currentColor" viewBox="0 0 20 20">
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
              </svg>
            ...
            {ratingBadge && !compact && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-keepr-evergreen/10 text-keepr-evergreen text-[11px] font-semibold">
                {ratingBadge}
              </span>
            )}
          </div>
        ...
        {/* Name */}
        <h3
          className={`${compact ? "text-base" : "text-lg"} font-bold text-foreground mb-1 line-clamp-1`}
        >
          {name}
        </h3>
        ...
        {/* Location */}
        <p
          className={`text-sm text-muted-foreground ${compact ? "mb-2" : "mb-3"} flex items-center gap-1`}
        >
          <svg
        ...
        {compact ? (
          <div className="flex items-center justify-between">
            ...
            <Link
              href={campgroundPath}
              onClick={onExplore}
              className="text-sm font-semibold text-keepr-evergreen hover:text-keepr-evergreen-light"
            >
              View →
            </Link>
          </div>
          ...
          <div className="flex items-center justify-between pt-3 border-t border-border">
              ...
              <div className="group/price relative">
                ...
                {totalPriceEstimate && (
                  <div className="text-xs text-muted-foreground mt-0.5 cursor-help">
                    <span className="font-medium text-keepr-evergreen">
                      ${totalPriceEstimate.total} total
                    </span>
                    <span className="text-muted-foreground">
                      {" "}
                      for {totalPriceEstimate.nights} nights
                    </span>
                    ...
                    <div className="absolute left-0 bottom-full mb-2 w-48 p-3 bg-muted text-foreground text-xs rounded-lg shadow-xl opacity-0 invisible group-hover/price:opacity-100 group-hover/price:visible transition-all z-20">
                      <div className="flex justify-between mb-1">
                        <span>
                          ${pricePerNight} x {totalPriceEstimate.nights} nights
                        </span>
                      ...
                      {showNoHiddenFeesBadge && (
                        <div className="mt-2 pt-2 border-t border-border flex items-center gap-1 text-keepr-evergreen">
                ...
                {showNoHiddenFeesBadge && !totalPriceEstimate && (
                  <div className="flex items-center gap-1 text-xs text-keepr-evergreen mt-0.5">
                    <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                      <path
                        fillRule="evenodd"
                        d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
            ...
            {isInternal ? (
              <Link
                href={campgroundPath}
                onClick={onExplore}
                className="px-4 py-2 text-sm font-semibold text-keepr-evergreen hover:text-keepr-evergreen-light hover:bg-keepr-evergreen/10 rounded-lg transition-colors flex items-center gap-1 group"
              >
