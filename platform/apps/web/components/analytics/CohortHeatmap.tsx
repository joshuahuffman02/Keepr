          ...
          <div className="animate-pulse space-y-3">
            ...
            {[...Array(5)].map((_, i) => (
              <div key={i} className="flex gap-2">
                ...
                {[...Array(5)].map((_, j) => (
                  <div key={j} className="h-10 w-16 bg-muted rounded" />
                ))}
              </div>
            ))}
          </div>
      ...
      <CardContent>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr>
                <th className="text-left py-3 px-3 text-sm font-medium text-muted-foreground min-w-[140px]">
                  Cohort
                </th>
                {labels.map((label, idx) => (
                  <th
                    key={idx}
                    className="text-center py-3 px-2 text-sm font-medium text-muted-foreground min-w-[70px]"
                  >
                    {label}
                  </th>
                ))}
            ...
            <tbody>
              {data.map((row, rowIdx) => (
                <tr key={rowIdx}>
                  <td className="py-2 px-3 text-sm font-medium text-foreground whitespace-nowrap">
                    {row.cohort}
                  </td>
