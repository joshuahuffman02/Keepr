...
export function CheckoutModal({
  ...
  const offlineCardWarning =
    !isOnline && (method === "card" || method === "terminal" || method === "saved_card");
  ...
  useEffect(() => {
    ...
    apiClient
      .getTerminalReaders(campgroundId)
      .then((data) => {
        setReaders(data);
  ...
  useEffect(() => {
    ...
    apiClient
      .getChargeablePaymentMethods(campgroundId, guestId)
      .then((data) => {
        setSavedCards(data);
    ...
    try {
      ...
      if (method === "terminal") {
        ...
        recordTelemetry({
          ...
          type: "sync",
          status: "success",
          message: "Terminal payment processed",
          meta: { items: cart.length, paymentMethod: "terminal" },
        });
      ...
      if (method === "saved_card") {
        ...
        recordTelemetry({
          ...
          type: "sync",
          status: "success",
          message: "Saved card charged",
          meta: { items: cart.length, paymentMethod: "saved_card" },
        });
        ...
        <div className="space-y-6 py-4">
          ...
          {(fulfillment !== "pickup" || method === "charge_to_site") && (
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Required for delivery, curbside, or table service. Charge-to-site also accepts guest
                name.
              </p>
            </div>
          )}
                ...
                <SelectContent>
                  {savedCards.map((card) => (
                    <SelectItem key={card.id} value={card.id}>
                      {card.brand} ···· {card.last4} {card.nickname && `(${card.nickname})`}{" "}
                      {card.isDefault && "★"}
                    </SelectItem>
                  ))}
                </SelectContent>
          ...
          {offlineCardWarning && (
            <div className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">
              Offline: card payments can't be processed right now. Switch to cash / charge to site
              or wait until you're back online.
            </div>
          )}

          {method === "guest_wallet" && walletBalanceCents < totalCents && (
            <div className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">
              Wallet balance (${(walletBalanceCents / 100).toFixed(2)}) is less than total ($
              {(totalCents / 100).toFixed(2)}). Please use a different payment method or split
              payment.
            </div>
          )}
