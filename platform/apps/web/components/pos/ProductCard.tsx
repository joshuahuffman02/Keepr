...
export function ProductCard({ product, onClick }: ProductCardProps) {
  ...
  const hasPriceOverride =
    product.effectivePriceCents !== undefined && product.effectivePriceCents !== product.priceCents;
  ...
  const lowStock =
    ...
    (hasEffectiveStock
      ...
      : (mode === "split" ? Math.min(posStock, onlineStock) : sharedStock) <=
  ...
  return (
    <button
      ...
      aria-label={
        hasStock
          ? `Add ${product.name} to cart, $${(displayPrice / 100).toFixed(2)}`
      ...
      className={cn(
        ...
        hasStock &&
          "hover:shadow-lg hover:border-emerald-200 hover:-translate-y-0.5 active:scale-[0.97] active:shadow-sm cursor-pointer",
      ...
      <div className="p-4 flex-1 flex flex-col justify-between w-full">
        <div className="flex items-start justify-between gap-2">
          ...
          {product.afterHoursAllowed && (
            <Badge
              variant="outline"
              className="text-amber-700 border-amber-300 bg-amber-50 text-[10px] h-6"
            >
              After Hours
            </Badge>
          )}
