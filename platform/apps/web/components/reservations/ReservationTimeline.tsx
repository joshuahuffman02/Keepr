...
export function ReservationTimeline({ reservation }: ReservationTimelineProps) {
  ...
  const firstPaymentDate = payments.length > 0 ? normalizeDate(payments[0].createdAt) : null;
        ...
        <div className="relative space-y-6 pl-2">
          {events.map((event, i) => (
            <div key={i} className="relative flex gap-4">
              ...
              <div
                className={`relative z-10 w-10 h-10 rounded-full flex items-center justify-center ${event.bg} border-2 border-card shadow-sm`}
              >
