...
export default function () {
  const res = http.post(`${__ENV.API_BASE}/pricing/rules`, { /* minimal payload */ });
...
describe('@smoke-hardening pricing-change', () => {
  it('propagates updated price to listing', () => {
    ...
    cy.get('[data-testid="price-value"]').invoke('text').should(text => {
...
export class RedactingLogger implements LoggerService {
  log(message: string, ...optional: any[]) { console.log(redact(message), ...optional.map(redact)); }
  error(message: string, ...optional: any[]) { console.error(redact(message), ...optional.map(redact)); }
  warn(message: string, ...optional: any[]) { console.warn(redact(message), ...optional.map(redact)); }
  debug(message: string, ...optional: any[]) { console.debug(redact(message), ...optional.map(redact)); }
  verbose(message: string, ...optional: any[]) { console.debug(redact(message), ...optional.map(redact)); }
}
```
